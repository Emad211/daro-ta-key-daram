...
The widget suite must fail on uncaught Flutter layout, rendering, or semantics exceptions. Primary actions must remain reachable by scrolling; tests must not reduce system text scale to make a control fit.

## Product rules

- Persian RTL is the canonical product direction.
- Icon-only interactive controls require meaningful Persian tooltips, which also provide semantic labels.
...
- Horizontal action groups must adapt vertically when text or available width cannot fit.
- Dialog content that can grow with system text must remain scrollable.
...
Record device model, Android version, display size, font size, locale, and result for every manual run.
