...
void main() {
  ...
  group('DriftMedicationRepository with an in-memory database', () {
    ...
    setUp(() {
      ...
      repository = DriftMedicationRepository(database, clock: () => now);
    });
    ...
    test('uses the newest inventory event as calculation baseline', () async {
      await repository.upsert(_medication(now: now));
      final DateTime restockedAt = now.add(const Duration(days: 2));
      ...
      expect(stored?.inventoryRecordedAt.toUtc(), restockedAt);
      expect(
